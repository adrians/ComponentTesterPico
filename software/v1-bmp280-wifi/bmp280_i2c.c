...
#define WIFI_SSID "NiceTry"
#define WIFI_PASSWORD "NiceTry"
...
int main() {
    ...
    ip_addr_t srv_addr;
    IP4_ADDR(&srv_addr, 192, 168, 111, 118);
    ...
    while (1) {
        ...
        snprintf(text_buffer, 200, "weather pressure=%.3f,temp=%.2f\n", pressure / 100.f, temperature / 100.f);

        cyw43_arch_lwip_begin();
            struct pbuf *p = pbuf_alloc(PBUF_TRANSPORT, strlen(text_buffer), PBUF_RAM);
            uint8_t *req = (uint8_t *) p->payload;
            memcpy(req, text_buffer, strlen(text_buffer)); // put payload in req
            udp_sendto(pcb, p, &srv_addr, 8094);
            pbuf_free(p);
        cyw43_arch_lwip_end();
